...
def register_callbacks(dashapp):
    @dashapp.callback(
        ...
        State('disable_dropdown','value'),
        State('period_dropdown','value'),
         State('stock_graph','figure')]


    )

    def update_stock_graph(show_button, company, start_date, end_date, radio_value, period_value, figure):

        if (show_button>0) and (radio_value=='stock_date'):
        ...
        elif (show_button > 0) and (radio_value == 'period'):
            if period_value=='max':
                price_df = pd.DataFrame(columns=["trade_date", "close"])
